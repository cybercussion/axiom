...
| **Not supported** | Nested *layouts* — a parent route rendering a shell around a child. Also **a literal multi-segment route key** such as `'scobot/packager'`: patterns are matched only when they contain a `:param`, so a literal key never matches and the mounted element comes from the first segment alone (`<scobot-ui>`). Use `:param` segments, or track `p-J4442V8WB3`. Hash routing; server rendering; a "leave this page?" guard; preload hints. The router carries nothing of the app's: page titles, the app name and the login path are all options. |
