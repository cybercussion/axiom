...
- **Added:** route changes are announced to screen readers (`@core/announce.js`, reconciled from the fleet's never-imported announce-bus); a template `/login` (reconciled from a downstream project) that says so when no provider is configured — the dock's Login link no longer lands on the 404; `rel="external"` opts a same-origin link out of client routing (reconciled from a downstream project).
...
- **Added:** a bound on a view transition the browser never starts — after 1 s the router skips it and commits. (`1314916`) It did **not** cure the WebKit-on-Linux CI stall that prompted it: the traces of runs 34629597306 and 34635441704 show no timer firing for 5–9 s — the bound's own included — while a navigation starts a transition from `/components`. The page's main thread is blocked, most likely capturing the transition on the GPU-less runner; local WebKit takes milliseconds. The browser suite gives WebKit's assertions room instead. (`601e789`)
