...
- `src/shared/base-component.js` — `BaseComponent`: Shadow DOM; lifecycle `setup()` → `render()` → `onRendered()`; a `rendered` promise the router awaits; `this.subscribe(key, cb)` (auto-cleanup); adopts `theme.css` into the shadow root. Also in `shared/`: modal, toast-manager, form-validator, focus-registry, announce-bus, keyboard-policy.
- `src/features/<name>/<name>.js` — a feature IS a Web Component: `customElements.define('<name>-ui', class extends BaseComponent {…})`.
- `tools/minify.js` — the **canonical fleet build** (see `docs/patterns/stale-deploy-prevention.md`). Non-destructive src → dist; stamps `?v=BUILD_ID`; three hard-fail guards (`assertNoSrcReferencesInDist`, `assertNoAliasConflicts`, `assertSingleCacheBustId`). Tests: `npm run test:tools` — run them after ANY edit to minify.js. It has a **`BUILD_PROFILE`** switch: `'web'` (main — absolute paths, import-map aliases resolved away by the build, `?v=` on all six module surfaces) vs `'scorm'` (scobot-player branches — a SCO runs from an arbitrary LMS content path, so dist uses `<base href="./">`, everything RELATIVE, the import map stays **live** at runtime with aliases NOT resolved, and `@scobot` is copied from node_modules to `dist/lib/`. `npm run scorm[:12|:2004]` zips dist into a SCORM package). Never mix the lanes: absolute paths break the SCORM package; unresolved aliases regress the website's cache-busting.
