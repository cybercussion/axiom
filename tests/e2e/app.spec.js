...
test('fixed chrome does not animate during a navigation (the dock fade nobody could see)', async ({ page, browserName }) => {
  test.skip(browserName !== 'chromium', 'view transitions: Chromium is the engine that runs them here');
  // Reported downstream as "page transitions made the dock fade out". A transition
  // captures the old ROOT, fixed chrome is inside it, and the root's pseudos are
  // animated — so the dock cross-faded on every navigation. The element's own
  // opacity stays 1 throughout, which is why no assertion on the element can see
  // it: the fade lives in the pseudo. So assert on the pseudos instead.
  await page.goto('/');
  await settled(page, 'home-ui');
  ...
  expect(named, 'fixed chrome must carry its own view-transition-name').toContain('navigation');
  ...
  const animating = await page.evaluate(async () => {
    ...
    const tick = () => {
      for (const a of document.getAnimations()) {
        const p = String(a.effect?.pseudoElement || '');
        if (p.includes('view-transition')) seen.add(p);
      }
  ...
  expect(animating.some((p) => p.includes('main-content')), `the page content should animate — saw ${animating.join(', ')}`).toBe(true);
  expect(animating.filter((p) => p.includes('(navigation)')), 'the dock must not animate').toEqual([]);
});
